...
def create_sample_excel_files():
    ...
    sales_data = {
        ...
        'Cliente': [
            f'Cliente_{i}'
            for i in np.random.randint(1, 200, 1000)
        ],
    ...
    inventory_data = {
        ...
        'Prezzo_Acquisto': np.round(
            np.random.uniform(5, 800, len(products) * 49),
            2
        ),
        'Prezzo_Vendita': np.round(
            np.random.uniform(10, 1200, len(products) * 49),
            2
        ),
    ...
    print(
        f"Creato: test_inventario.xlsx ({len(df_inventory)} righe)"
    )
