...
__all__ = ['AdvancedAIQueryInterpreter']
# flake8: noqa
...
class AdvancedAIQueryInterpreter:
    ...
    def set_data_context(self, current_data, imported_files):
        ...
        try:
            ...
            if any(word in input_lower for word in ['mostra', 'show', 'seleziona', 'select', 'visualizza', 'dammi']):
                ...
                if mentioned_cols:
                    ...
                    if any(word in input_lower for word in ['tutto', 'all', 'tutti', 'completo']):
    ...
    def _generate_group_query(self, input_text: str, text_cols: List[str], numeric_cols: List[str], table_name: str) -> str:
        if text_cols and numeric_cols:
            return (
                f"-- Raggruppa per {text_cols[0]}, aggrega {numeric_cols[0]}\n"
                f"SELECT [{text_cols[0]}], AVG([{numeric_cols[0]}]) as Media, COUNT(*) as Conteggio "
                f"FROM [{table_name}] GROUP BY [{text_cols[0]}] ORDER BY Conteggio DESC;"
            )
        elif text_cols:
            return (
                f"-- Raggruppa per {text_cols[0]}\n"
                f"SELECT [{text_cols[0]}], COUNT(*) as Conteggio FROM [{table_name}] GROUP BY [{text_cols[0]}] ORDER BY Conteggio DESC;"
            )
        else:
            return f"-- Nessuna colonna testuale per raggruppare\nSELECT COUNT(*) as Totale_Righe FROM [{table_name}];"

    def _generate_sort_query(self, input_text: str, columns: List[str], numeric_cols: List[str], text_cols: List[str], table_name: str) -> str:
        col = None
        for c in columns:
            if c.lower() in input_text:
                col = c
                break
        if not col:
            col = numeric_cols[0] if numeric_cols else (text_cols[0] if text_cols else columns[0])
        order = 'ASC'
        if any(word in input_text for word in ['desc', 'decrescente', 'descending']):
            order = 'DESC'
        elif any(word in input_text for word in ['asc', 'crescente', 'ascending']):
            order = 'ASC'
        return (
            f"-- Ordina per {col} ({order})\n"
    ...
    def _generate_stats_query(self, input_text: str, numeric_cols: List[str], text_cols: List[str], table_name: str) -> str:
        ...
        if any(word in input_text for word in ['somma', 'sum', 'totale']):
            if numeric_cols:
        ...
        if any(word in input_text for word in ['conta', 'count', 'numero']):
            if text_cols:
